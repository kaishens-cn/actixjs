...
use crate::{
  extras::scheduler::{pin_js_thread, reset_thread_affinity, try_pin_priority},
  object_pool::{build_up_pool, get_stored_chunk, StoredPair},
  request::helpers::make_js_error,
...
use super::{
  ...
  shutdown::{attach_server_handle, try_own_start},
};
...
impl ServiceFactory<Request> for AppFactory {
  ...
  fn new_service(&self, _: ()) -> Self::Future {
    ...
    Box::pin(async move {
      Ok(ActixHttpServer {
        _hdr_srv: HeaderValue::from_static("Actix"),
...
pub fn start_server(config: ServerConfig, env: sys::napi_env) -> napi::Result<()> {
  ...
  initialise_reader();
  unsafe {
    build_up_pool(env, config.get_pool_size())?;
  }
