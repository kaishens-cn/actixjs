...
impl ServerConfig {
  ...
  pub fn from_config_blob(config: HashMap<String, String>) -> Result<Self> {
    ...
    let get_number_with_deault = |key: &'static str, fallback: usize| match config.get(key) {
      Some(res) => match res.parse::<usize>() {
        Ok(res) => Ok(res),
        Err(_) => Err(make_js_error_string(format!(
          "Invalid number provided for {}",
          key
        ))),
      },
      None => Ok(fallback),
    ...
    let get_bool_with_default = |key: &'static str, fallback: bool| match config.get(key) {
    ...
    if get_bool_with_default("tls", false)? {
      ...
      if key_location.is_none() || cert_location.is_none() {
        return Err(make_js_error(
          "We need both the key and cert location for TLS",
        ));
      }
    }
