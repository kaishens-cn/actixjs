...
static ACTIX_SERVER: HeaderValue = HeaderValue::from_static("actix");
...
static RAW_HEADER_VAL: HeaderValue =
...
fn render_internal_error_with_bytes(message: Bytes) -> Response<Bytes> {
  Response::with_body(StatusCode::INTERNAL_SERVER_ERROR, message)
}
...
fn apply_headers(
  ...
  hdrs.insert(SERVER, ACTIX_SERVER.clone());
